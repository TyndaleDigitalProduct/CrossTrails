...
import { CrossReferenceAnalysisService, analyzeCrossReference } from '@/lib/llm'
...
async function basicAnalysisExample() {
  ...
  try {
    const result = await analyzeCrossReference({
      ...
      userObservation: 'I notice both verses talk about Gods unconditional love for sinners'
    })
    ...
    console.log('\n--- Sources Used ---')
    console.log(`Anchor: ${result.sources.anchor_verse.reference} - "${result.sources.anchor_verse.text}"`)
    console.log(`Cross-ref: ${result.sources.cross_reference.reference} - "${result.sources.cross_reference.text}"`)
...
async function glooAIAnalysisExample() {
  ...
  try {
    const result = await service.analyzeCrossReference({
      ...
      contextRange: 3,
      userObservation: 'How do these passages relate to Pauline theology about justification?'
    })
...
async function providerComparisonExample() {
  ...
  for (const config of providers) {
    ...
    if (connectionTest.success) {
      ...
      console.log(`✗ ${config.provider} connection failed: ${connectionTest.error}`)
    }
  }
}
...
async function robustAnalysisExample() {
  ...
  const fallbackConfig: LLMClientConfig = { provider: 'openai', model: 'gpt-4o-mini' }
