...
async function exampleBasicUsage() {
  ...
  try {
    const result = await generateCrossReferencePrompt({
      crossReference,
      userObservation:
        "I notice both verses emphasize that God's love is not conditional on our righteousness",
      contextRange: 2,
    });
...
async function exampleStudyTemplate() {
  const crossReference: CrossReference = {
    ...
    connection: {
      ...
      explanation:
        'Both passages describe the incarnation and humiliation of Christ',
    },
    reasoning:
      'These passages complement each other in describing the mystery of the incarnation - John emphasizing the Word becoming flesh, Philippians detailing the self-emptying (kenosis) of Christ in His incarnation and crucifixion.',
  };
...
async function exampleDevotionalTemplate() {
  const crossReference: CrossReference = {
    ...
    connection: {
      ...
      explanation:
        "Both passages use shepherd imagery to describe God's care and provision",
    },
    reasoning:
      "The shepherd metaphor connects these passages, with Psalm 23 expressing trust in God's provision and John 10 revealing Jesus as the Good Shepherd who lays down His life for the sheep.",
  };
...
async function exampleAcademicTemplate() {
  const crossReference: CrossReference = {
    ...
    reasoning:
      "This is a clear case of New Testament use of Old Testament prophecy, where Peter directly applies Isaiah's Suffering Servant passage to Christ's crucifixion, demonstrating the early Christian understanding of Jesus as the fulfillment of Isaiah's prophecy.",
  };
...
async function exampleWithRealData() {
  ...
  const crossReference: CrossReference = {
    ...
    reasoning:
      "These Gospel accounts provide complementary perspectives on the nativity, with Matthew focusing on the wise men's visit and Luke detailing the circumstances of the birth itself, both confirming Bethlehem as the birthplace.",
  };

  const result = await generateCrossReferencePrompt({
    crossReference,
    userObservation:
      'Why do both Matthew and Luke emphasize Bethlehem specifically?',
    contextRange: 1,
  });
  ...
  console.log(
    `- Connection strength: ${result.sources.connection_data.strength * 100}%`
  );
  console.log(
    `- Categories: ${result.sources.connection_data.categories.join(', ')}`
  );
  console.log('\nGenerated prompt:');
