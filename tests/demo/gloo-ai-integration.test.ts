  ...
  it('should send prompt to Gloo AI and get response for Matt.2.1 -> Luke.1.5', async () => {
    ...
    const crossReference: CrossReference = {
      ...
      connection: {
        ...
        explanation: 'Both passages reference the reign of King Herod as historical context for the birth narrative'
      },
      reasoning: 'PARALLEL_ACCOUNT: Same gospel event. Shared names/narrative structure. Both passages establish the historical timeframe during Herod\'s reign.'
    }

    console.log('\\n=== TESTING GLOO AI INTEGRATION ===')
    console.log(`Cross-Reference: ${crossReference.anchor_ref} → ${crossReference.reference}`)
    console.log(`Connection: ${crossReference.connection.type} (${crossReference.connection.strength * 100}% strength)`)
    ...
    try {
      ...
      console.log(`Connection Status: ${connectionTest.success ? 'SUCCESS' : 'FAILED'}`)
      ...
      if (connectionTest.success) {
        ...
        const result = await analysisService.analyzeCrossReference({
          ...
          userObservation: 'I\'m studying the birth narrative and want to understand how Matthew and Luke complement each other.',
          contextRange: 2,
        ...
        console.log(`Tokens Used: ${result.llm_metadata.usage?.total_tokens || 'N/A'}`)
        console.log(`Prompt Tokens: ${result.llm_metadata.usage?.prompt_tokens || 'N/A'}`)
        console.log(`Completion Tokens: ${result.llm_metadata.usage?.completion_tokens || 'N/A'}`)
        console.log(`Actual Time: ${endTime - startTime}ms`)

        console.log('\\n=== PROMPT VERIFICATION ===')
        console.log(`Prompt Length: ${result.prompt_used.length} characters`)
        console.log(`Contains Matt.2.1: ${result.prompt_used.includes('Matt.2.1')}`)
        console.log(`Contains Luke.1.5: ${result.prompt_used.includes('Luke.1.5')}`)
        ...
        console.log('This might be due to missing credentials or network issues')
      ...
      console.error('\\n❌ Error during Gloo AI testing:', error instanceof Error ? error.message : String(error))
      
      // Don't fail the test for connection issues, just report them
      console.log('\\nNote: This error might be due to:')
      console.log('- Missing GLOO_CLIENT_ID or GLOO_CLIENT_SECRET environment variables')
