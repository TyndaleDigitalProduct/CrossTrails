...
describe('getVerseContext', () => {
  ...
  describe('basic verse fetching', () => {
    it('should fetch a single verse without context', async () => {
      ...
      expect(mockNltClient.getVersesByReference).toHaveBeenCalledWith(
        'John.3.16'
      );
    });
  ...
  describe('error handling', () => {
    it('should handle invalid reference format', async () => {
      ...
      await expect(getVerseContext(request)).rejects.toThrow(
        'Failed to fetch verse context'
      );
    });
    ...
    it('should use fallback method for context if chapter fetch fails', async () => {
      ...
      mockNltClient.getVersesByReference
        ...
        .mockResolvedValueOnce({
          book: 'John',
          chapter: 3,
          verses: [mockBeforeVerse],
        })
        .mockResolvedValueOnce({
          book: 'John',
          chapter: 3,
          verses: [mockAfterVerse],
        });

      mockNltClient.getVersesByChapter.mockRejectedValue(
        new Error('Chapter fetch failed')
      );
