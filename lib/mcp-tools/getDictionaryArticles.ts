...
export async function getDictionaryArticles(
  ...
  try {
    ...
    const targetConcepts =
      concepts.length > 0 ? concepts : await extractConceptsFromVerses(verses);
    ...
    articles.sort((a, b) => {
      const avgA =
        a.relevance_to_verses.reduce(
          (sum, rel) => sum + rel.relevance_score,
          0
        ) / a.relevance_to_verses.length;
      const avgB =
        b.relevance_to_verses.reduce(
          (sum, rel) => sum + rel.relevance_score,
          0
        ) / b.relevance_to_verses.length;
      return avgB - avgA;
    });
  ...
  return {
    articles: {
      King: {
        ...
        title: 'King',
        summary:
          'In biblical usage, king refers to both earthly rulers and the ultimate divine King',
        content:
          'In the biblical world, kings were seen as representatives of divine authority on earth. The concept of kingship in Scripture encompasses both human rulers like David and Herod, and ultimately points to the divine kingship of God and the Messianic King, Jesus Christ. The wise men\'s search for the "newborn king of the Jews" reflects their understanding of Jesus\' royal nature.',
      ...
      Star: {
        ...
        title: 'Star',
        summary:
          'Stars in biblical literature often symbolize divine guidance and messianic hope',
        content:
