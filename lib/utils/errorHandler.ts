...
class ErrorHandler {
   ...
   */
  createValidationError(field: string, message: string, details?: any): APIException {
  ...
  private classifyError(error: Error): { code: ErrorCode; message: string; statusCode: number; suggestion?: string } {
    ...
    if (message.includes('nlt') || message.includes('bible api')) {
      return {
        ...
        statusCode: 503,
        suggestion: 'The Bible text service is temporarily unavailable. Please try again in a few minutes.'
      }
    }
    ...
    if (message.includes('timeout') || message.includes('timed out')) {
      return {
        ...
        statusCode: 408,
        suggestion: 'The request took too long to complete. Try simplifying your request or try again later.'
      }
    }
    ...
    if (message.includes('rate limit') || message.includes('too many requests')) {
      return {
        ...
        statusCode: 429,
        suggestion: 'You have made too many requests. Please wait before making additional requests.'
      }
    }
    ...
    if (message.includes('invalid') || message.includes('validation') || message.includes('required')) {
      return {
        ...
        statusCode: 400,
        suggestion: 'Please check your request parameters and ensure they are valid'
      }
    }
  ...
  private logError(error: APIError, context: ErrorContext, originalError: unknown): void {
    // In production, this would integrate with a logging service like Winston, Sentry, etc.
    console.error('API Error:', {
      ...error,
      context,
      original_error: originalError instanceof Error ? {
