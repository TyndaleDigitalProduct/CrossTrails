const BIBLE_BOOKS = [
  ...
  {
    name: 'Song of Songs',
    abbrevs: ['Song', 'So', 'SOS', 'Song of Solomon', 'SoS', 'Canticles'],
  },
  ...
  {
    name: '1 John',
    abbrevs: ['1Jn', '1 Jn', '1J', '1 J', 'I John', 'I Jn', 'I J', '1John'],
  },
  {
    name: '2 John',
    abbrevs: ['2Jn', '2 Jn', '2J', '2 J', 'II John', 'II Jn', 'II J', '2John'],
  },
  {
    name: '3 John',
    abbrevs: [
 ...
 */
export function findBookInString(
  input: string
): { book: string; remaining: string } | null {
 ...
 */
export function findChapterInString(
  bookName: string,
  input: string
): { chapter: number; remaining: string } | null {
  const normalized = input?.toLowerCase()?.trim();

  const chapterMatch = /\b(\d+)\b/.exec(normalized);
  if (chapterMatch) {
    const chapter = parseInt(chapterMatch[1], 10);
    const remaining = normalized.substring(
