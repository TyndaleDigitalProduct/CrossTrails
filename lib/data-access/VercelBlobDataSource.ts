...
async getChapterData(bookAbbrev: string, chapter: number): Promise<
...
> {
  try {
    ...
    console.error(`[VercelBlobDataSource] Error loading blob for chapter "${bookAbbrev}.${chapter}":`, error)
    return []
  }
}
...
async getBookData(bookAbbrevation: string): Promise<CrossReferenceDataFile | null> {
  try {
    ...
    const stdBookAbbrev = fullBookNameToAbbrev[bookAbbrevation] || bookAbbrevation
