...
export class CrossReferenceAnalysisService {
   ...
   */
  async *analyzeCrossReferenceStream(
    request: CrossReferenceAnalysisRequest
  ): AsyncGenerator<{
    content: string;
    done: boolean;
    ...
    try {
      ...
      throw new Error(
        `Streaming analysis failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private getSystemPrompt(analysisType: string): string {
    ...
    switch (analysisType) {
      case 'study':
        return (
          basePrompt +
          ` Focus on providing educational insights suitable for Bible study groups. Include historical context, literary analysis, and practical applications. Structure your response clearly with headings and key points.`
        );

      case 'devotional':
        return (
          basePrompt +
          ` Focus on personal, spiritual insights that encourage faith and practical Christian living. Be warm, encouraging, and application-focused while maintaining biblical accuracy.`
        );

      case 'academic':
        return (
          basePrompt +
          ` Provide scholarly analysis with attention to original languages, historical-critical methods, and theological implications. Reference scholarly consensus and note any significant interpretive debates.`
        );

      default:
        return (
          basePrompt +
          ` Provide balanced analysis that combines scholarly insight with practical application. Make complex theological concepts accessible to a general audience, and lead users to their own conclusions rather than telling them the correct answers directly.`
        );
    }
  }
}
...
export async function analyzeCrossReference(
  request: CrossReferenceAnalysisRequest
): Promise<CrossReferenceAnalysisResponse> {
