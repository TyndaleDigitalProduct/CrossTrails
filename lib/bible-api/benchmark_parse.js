...
function parseRegex(html, referenceHint) {
  ...
  while ((m = verseRegex.exec(html))) {
    ...
    const bk = bkMatch ? bkMatch[1][0].toUpperCase() + bkMatch[1].slice(1) : undefined
    ...
    const cleaned = parts.map((part) => htmlToText(part)).map((t) => t.replace(/\*+/g, ' ')).filter(Boolean).join(' ')
...
function parseDOM(html, referenceHint) {
  ...
  for (const veNode of nodes) {
    ...
    Array.from(ve.querySelectorAll('[class*="tn-ref"], [class*="tn"]')).forEach(n => n.remove())
...
async function run() {
  ...
  console.log('Regex total ms:', regexMs.toFixed(2), 'avg ms:', (regexMs / ITERS).toFixed(4))
  console.log('DOM total ms:  ', domMs.toFixed(2), 'avg ms:', (domMs / ITERS).toFixed(4))
