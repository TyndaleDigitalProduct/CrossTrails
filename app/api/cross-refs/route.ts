      ...
      return NextResponse.json(
        {
          error: {
            ...
            message: 'Must provide either "verse", "verses", or "chapter" parameter',
...
async function getCrossReferencesForVerse(
  ...
  try {
    ...
    const connections = (Array.isArray(mcpResponse[0]) ? mcpResponse[0] : mcpResponse) as any[]
...
async function getCrossReferencesForChapter(
  chapter: string,
  minStrength: number
): Promise<Array<{ anchor_verse: string; cross_references: CrossReference[] }>> {
  try {
    ...
    const results = mcpResponse.map((connections: any[]) => {
    ...
    }).filter((result): result is { anchor_verse: string; cross_references: CrossReference[] } => result !== null)
