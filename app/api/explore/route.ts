    ...
    if (selectedVerses.length > 0) {
      ...
      if (selectedVerses.length > 0) {
        ...
        if (flatConnections && flatConnections.length > 0) {
          ...
          // Group by strength
          const strongConnections = flatConnections.filter(
            c => c.strength >= 0.8
          );
          const moderateConnections = flatConnections.filter(
            c => c.strength >= 0.6 && c.strength < 0.8
          );
